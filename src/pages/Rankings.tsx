  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (data) {
        ...
        (data as any[]).forEach(s => { const e = map.get(s.phone) || { hours: 0, sessions: 0 }; e.hours += (s.duration_used || 0) / 60; e.sessions += 1; map.set(s.phone, e); });
        setRankings(Array.from(map.entries()).map(([phone, v]) => ({ phone, hours: Math.round(v.hours * 10) / 10, sessions: v.sessions })).sort((a, b) => b.hours - a.hours));
      }
      setLoading(false);
    };
    fetch();
  ...
  return (
    <AdminLayout>
      ...
      {loading ? (
        ...
        <div className="glass-card p-12 text-center"><Trophy className="w-12 h-12 mx-auto text-muted-foreground/50 mb-3" /><p className="text-sm text-muted-foreground">No rankings yet</p></div>
      ) : (
        <>
          {rankings.length >= 3 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {rankings.slice(0, 3).map((u, i) => (
                <div key={i} className={`glass-card p-6 text-center ${i === 0 ? "sm:order-2 ring-1 ring-primary/30" : i === 1 ? "sm:order-1" : "sm:order-3"}`}>
