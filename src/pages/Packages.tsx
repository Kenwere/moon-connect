...
interface Pkg {
  ...
  duration_label: string; speed_limit: string | null; data_limit: string | null; active: boolean;
}
...
export default function Packages() {
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from("packages").insert({
      user_id: user.id, org_id: orgs?.[0]?.id || null, name: form.name, price: Number(form.price),
    ...
    <AdminLayout>
      <PageHeader title="Packages" subtitle="Manage internet access packages">
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-primary text-primary-foreground font-medium"><Plus className="w-4 h-4 mr-2" /> Add Package</Button>
          </DialogTrigger>
          <DialogContent className="glass-card border-border">
            ...
            <div className="space-y-4 mt-4">
              ...
              <Button className="w-full bg-primary text-primary-foreground" onClick={handleAdd}>Create Package</Button>
            </div>
          </DialogContent>
        </Dialog>
      </PageHeader>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {packages.map((pkg) => (
            <div key={pkg.id} className={`glass-card p-5 transition-all ${!pkg.active && "opacity-50"}`}>
              ...
              <p className="text-3xl font-display font-bold text-primary mb-1">KSH {pkg.price}</p>
