      ...
      <PageHeader title="Vouchers" subtitle="Generate and manage WiFi access vouchers">
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button className="gradient-primary text-primary-foreground font-medium"><Plus className="w-4 h-4 mr-2" /> Generate Vouchers</Button>
          </DialogTrigger>
            ...
            <div className="space-y-4 mt-4">
              ...
              <div>
                ...
                <Select value={selectedPkg} onValueChange={setSelectedPkg}>
                  ...
                  <SelectContent>
                    {packages.map(p => <SelectItem key={p.id} value={p.id}>{p.name} - {p.duration_label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div><Label>Expiry Date</Label><Input type="date" className="mt-1.5" value={expiry} onChange={e => setExpiry(e.target.value)} /></div>
              <Button className="w-full gradient-primary text-primary-foreground" onClick={handleGenerate}>Generate</Button>
            </div>
          </DialogContent>
        </Dialog>
      </PageHeader>
                  ...
                  <tr key={v.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-1">
                        <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => copyCode(v.code)}><Copy className="w-3 h-3 mr-1" /> Copy</Button>
                      </div>
                    </td>
                  </tr>
                ))}
