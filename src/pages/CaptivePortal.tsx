        ...
        <AnimatePresence mode="wait">
          {step === "select" && (
            <motion.div key="select" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="space-y-3">
                {packages.map((pkg) => (
                  <button
                    ...
                    className={`w-full glass-card rounded-xl p-4 text-left transition-all duration-200 hover:scale-[1.02] ${
                      selected === pkg.id ? "ring-2 ring-primary shadow-lg shadow-primary/10" : ""
                    } ${pkg.popular ? "relative" : ""}`}
                  >
                    {pkg.popular && (
                      <span className="absolute -top-2 right-3 px-2 py-0.5 rounded-full text-[10px] font-bold gradient-primary text-primary-foreground">
                        POPULAR
                      </span>
                    )}
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-display font-bold text-lg gradient-text">KSH {pkg.price}</p>
                    </div>
                  </button>
                ))}
              </div>
              ...
              <Button
                className="w-full mt-4 gradient-primary text-primary-foreground font-medium h-12"
                disabled={!selected}
              ...
              <button
                ...
                className="w-full text-center text-xs text-muted-foreground mt-3 hover:text-primary transition-colors"
              >
                Have a voucher code?
              </button>
            ...
            <motion.div key="phone" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="glass-card rounded-xl p-5 mb-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p className="font-display font-bold gradient-text">KSH {selectedPkg?.price}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Button
                  className="w-full gradient-primary text-primary-foreground font-medium h-12"
                  disabled={phone.length < 10}
                  onClick={() => setStep("pay")}
                >
                  Pay with M-Pesa <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
                <button onClick={() => setStep("select")} className="w-full text-center text-xs text-muted-foreground hover:text-primary transition-colors">
                  ← Back to packages
                </button>
              </div>
            </motion.div>
          ...
          {step === "pay" && (
            <motion.div key="pay" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="text-center">
              <div className="glass-card-glow rounded-xl p-8">
                <div className="w-16 h-16 rounded-full bg-primary/15 flex items-center justify-center mx-auto mb-4 animate-pulse-glow">
                  <Wifi className="w-8 h-8 text-primary" />
                </div>
                <h3 className="font-display font-bold text-lg mb-1">Payment Request Sent</h3>
                <p className="text-sm text-muted-foreground mb-4">
                  Check your phone <span className="font-mono text-primary">{phone}</span> for the M-Pesa prompt
                </p>
                <div className="glass-card rounded-lg p-4 text-sm space-y-2 text-left mb-4">
              ...
              <button onClick={() => { setStep("select"); setSelected(null); setPhone(""); }} className="text-xs text-muted-foreground hover:text-primary transition-colors mt-4">
                ← Start over
              </button>
            </motion.div>
          )}
